class ApiClient {
  ...
  async getCalendarIntegrations(params?: {
    provider?: string;
    syncEnabled?: boolean;
    isActive?: boolean;
  }) {
    ...
    return this.request(`/calendar/integrations?${queryParams.toString()}`);
  }

  async getCalendarIntegration(id: string) {
    return this.request(`/calendar/integrations/${id}`);
  }

  async createCalendarIntegration(data: any) {
    return this.request('/calendar/integrations', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }

  async updateCalendarIntegration(id: string, data: any) {
    return this.request(`/calendar/integrations/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    });
  ...
  async syncBookingToCalendar(bookingId: string) {
    return this.request(`/calendar/sync/booking/${bookingId}`, {
      method: 'POST',
    });
  }
  ...
  async checkCalendarConflicts(startTime: string, endTime: string) {
    const queryParams = new URLSearchParams({
      startTime,
      endTime,
    });
    
    return this.request(`/calendar/conflicts?${queryParams.toString()}`);
  }
}
